...
Use `q1x discover --manifest <file-or-directory>` to populate the live registry, `q1x capabilities list` to inspect it, `q1x endpoints` / `q1x model invoke` for model transport, and `q1x browser-endpoints` / `q1x browser run` for one-shot browser automation. Use `q1x desktop-endpoints` / `q1x desktop run` for allowlisted native desktop control. Browser binaries are not bundled; `playwright-core` is the browser control library. See the repository documentation for discovery, model transport, browser control, desktop control, command examples and checkpoint semantics.
