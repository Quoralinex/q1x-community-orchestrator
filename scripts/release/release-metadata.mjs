...
export const PUBLIC_PACKAGES = [
  ...
  ['@quoralinex/q1x-community-desktop-bridge-common', 'packages/desktop-bridge-common'],
  ['@quoralinex/q1x-community-desktop-bridge-macos', 'packages/desktop-bridge-macos'],
  ['@quoralinex/q1x-community-desktop-bridge-windows', 'packages/desktop-bridge-windows'],
  ['@quoralinex/q1x-community-desktop-bridge-linux', 'packages/desktop-bridge-linux'],
...
export function assertReleaseIdentity(identity) {
  ...
  for (const packageInfo of identity.packages) {
    for (const dependency of Object.keys(packageInfo.dependencies ?? {})) {
      if (/@quoralinex\/q1x-(?:control-plane|sovereign|internal)/i.test(dependency)) {
...
export function buildReleaseManifest({ identity, sourceSha, artifacts, generatedAt }) {
  ...
  const expectedPackageNames = new Set(PUBLIC_PACKAGES.map(([name]) => name));
  const artifactPackageNames = new Set(normalizedArtifacts.map(artifact => artifact.packageName));
