...
Q1X Community Orchestrator remains experimental software intended for evaluation, local development and Community testing. Phase 12 is implemented on protected `main`. The historical `v0.1.0-alpha.1` release remains immutable. `v0.1.0-alpha.2` is now the commissioned GitHub public alpha after exact-head, post-merge and release-workflow validation. Its eight GitHub release tarballs, `release-manifest.json` and `SHA256SUMS` are the governed distribution evidence. npm publication remains optional and was not performed as part of this commissioning. The release is not production-ready or generally available.
